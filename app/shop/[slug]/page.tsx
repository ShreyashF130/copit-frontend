...
import { useParams } from 'next/navigation' // 👈 Safe Client-Side Routing
    ...
    <div className="min-h-screen bg-background pb-32 font-sans selection:bg-primary/20 animate-in fade-in duration-500">
        ...
        {filteredItems.length === 0 ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {filteredItems.map(item => {
              ...
              return (
                <div key={item.id} onClick={() => { setSelectedItem(item); setSelections({}); }} className="group bg-card rounded-[2rem] p-3 pb-5 flex flex-col gap-3 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer relative border border-border hover:border-primary/20 overflow-hidden">
      ...
      {selectedItem && (
        <div className="fixed inset-0 z-50 flex justify-center items-end sm:items-center p-0 sm:p-4">
          ...
          <div className="bg-card w-full max-w-lg sm:rounded-[2.5rem] rounded-t-[2.5rem] p-6 shadow-2xl relative animate-in slide-in-from-bottom-20 duration-300 flex flex-col max-h-[95vh] border border-border overflow-hidden">
            <button onClick={() => setSelectedItem(null)} className="absolute right-6 top-6 p-2 bg-secondbg/50 backdrop-blur-sm rounded-full text-muted-foreground hover:text-foreground z-20"><X size={20} /></button>
            
            {/* 🚨 THE FIX: Massive, detailed product showcase */}
            {/* Switched from containment to a responsive aspect ratio that maximizes the source pixels. */}
           <div className="w-full h-72 sm:h-96 rounded-3xl mb-6 overflow-hidden flex-shrink-0 relative border border-border flex items-center justify-center bg-black/5 dark:bg-white/5">
               {currentImage ? (
                   <img 
                     src={currentImage} 
                     alt={selectedItem.name}
                     className={`w-full h-full object-contain p-4 transition-all duration-500 hover:scale-105 ${isOutOfStock ? 'grayscale opacity-50' : ''}`}
                     onError={(e) => {
                        e.currentTarget.style.display = 'none';
                        if(e.currentTarget.nextElementSibling) {
                          (e.currentTarget.nextElementSibling as HTMLElement).style.display = 'flex';
               ...
               {isOutOfStock && <div className="absolute inset-0 flex items-center justify-center bg-background/60 backdrop-blur-[2px] z-10"><span className="bg-foreground text-background px-6 py-2 rounded-full font-black uppercase text-xl shadow-xl">Out of Stock</span></div>}
            </div>
            
            <div className="flex-1 overflow-y-auto pr-2 space-y-6 custom-scrollbar pb-4">
              <div>
                <div className="flex justify-between items-start gap-4">
                  <h2 className="text-2xl font-black tracking-tight text-foreground leading-tight w-3/4 line-clamp-2">{selectedItem.name}</h2>
                ...
                <div className="h-full flex flex-col items-center justify-center opacity-40 space-y-4">
                 ...
                 return (
                  <div key={c.id} className="flex gap-4 items-start py-2 border-b border-border last:border-0 pb-4">
                    <div className="w-20 h-20 rounded-2xl bg-secondbg overflow-hidden flex-shrink-0 border border-border flex items-center justify-center relative bg-black/5">
      ...
      <div className="mt-12 mb-8 p-6 bg-secondbg rounded-[2.5rem] text-center max-w-lg mx-auto border border-border">
