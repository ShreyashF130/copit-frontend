...
//         {/* 🚨 THE BLIND SPOT FIX: Alert if WhatsApp Failed */}
//         {order.notification_status === 'failed' && (
//           <div className="bg-red-500/10 border border-red-500/30 p-2 rounded-lg mb-2 animate-pulse">
//              <p className="text-[9px] font-bold text-red-500 flex items-center gap-1">
//                <AlertTriangle size={10} /> Receipt failed to send!
//              </p>
//           </div>
//         )}
        
//         {/* RESEND BUTTON: Turns Red to grab attention if it failed */}
//         <button 
...
//                 : 'bg-blue-50 text-blue-500 border-blue-100 hover:text-blue-700'
//             }`}
//         >
//             <MessageCircle size={10} /> {order.notification_status === 'failed' ? 'Retry Sending Receipt' : 'Resend Receipt'}
//         </button>
//      </div>
//   )}
...
//       order.status === 'PAID' || 
//       order.payment_method === 'COD' // 👈 THE FIX: COD bypasses the 'paid' requirement
//   ) ? (
//     <button 
...
//              className="w-full bg-white border border-border text-foreground px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase hover:bg-secondbg transition-colors flex items-center justify-center gap-2 shadow-sm"
//            >
...
import { 
  ...
  Clock, X, AlertTriangle, MessageCircle, FileText
} from 'lucide-react'
import { toast } from 'sonner'
import ShipmentModal from '@/app/components/shipmentModel'
    ...
    <div className="p-4 md:p-8 min-h-screen relative animate-in fade-in duration-500 pb-20">
                  ...
                  <tr key={order.id} className={`group transition-colors ${order.payment_status === 'needs_approval' ? 'bg-yellow-500/5 hover:bg-yellow-500/10' : 'hover:bg-secondbg/30'}`}>
                    ...
                    <td className="px-6 py-6 align-top min-w-[200px]">
                      ...
                      {order.payment_status === 'needs_approval' && (
                        <div className="space-y-3 animate-in slide-in-from-left-2">
                           ...
                           <div className="flex gap-2">
                             <button onClick={() => handlePaymentVerification(order.id, 'APPROVE')} className="flex-1 bg-green-600 text-white px-3 py-2 rounded-lg text-[9px] font-black uppercase hover:bg-green-700 shadow-md transition-all">Accept</button>
                             <button onClick={() => handlePaymentVerification(order.id, 'REJECT')} className="flex-1 bg-white border border-red-200 text-red-500 px-3 py-2 rounded-lg text-[9px] font-black uppercase hover:bg-red-50 transition-all">Reject</button>
                           </div>
                        </div>
                      )}
                      ...
                      {(order.payment_status === 'awaiting_screenshot' || order.payment_status === 'awaiting_proof' || !order.payment_status) && order.payment_method !== 'COD' && (
                         <div className="opacity-60">
                            <span className="text-[10px] font-bold text-slate-500 flex items-center gap-2 mb-2">
                               <Clock size={12} /> {order.payment_method === 'ONLINE' ? 'Waiting for Gateway...' : 'Waiting for user...'}
                            </span>
                         </div>
                      )}
                      ...
                      {(order.payment_status === 'paid' || order.status === 'PAID') && (
                         <div className="space-y-2">
                            ...
                            {order.notification_status === 'failed' && (
                              <div className="bg-red-500/10 border border-red-500/30 p-2 rounded-lg mb-2 animate-pulse">
                                 <p className="text-[9px] font-bold text-red-500 flex items-center gap-1"><AlertTriangle size={10} /> Receipt failed to send!</p>
                              </div>
                            )}
                            
                            <button 
                                onClick={() => handleResendMsg(order.id)}
                                className={`text-[9px] font-bold flex items-center gap-1 px-2 py-1 rounded border transition-colors ${order.notification_status === 'failed' ? 'bg-red-50 text-red-600 border-red-200 hover:bg-red-100' : 'bg-blue-50 text-blue-500 border-blue-100 hover:text-blue-700'}`}
                            >
                                <MessageCircle size={10} /> {order.notification_status === 'failed' ? 'Retry Sending Receipt' : 'Resend Receipt'}
                            </button>
                         </div>
                    ...
                    <td className="px-6 py-6 align-top text-right">
                      {order.delivery_status === 'processing' && (order.payment_status === 'paid' || order.status === 'PAID' || order.payment_method === 'COD') ? (
                        ...
                        <div className="flex flex-col items-end gap-2">
                            <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-1 border ${order.delivery_status === 'delivered' ? 'bg-green-50 text-green-600 border-green-200' : 'bg-blue-50 text-blue-600 border-blue-200'}`}>
                              <Truck size={12} /> {order.delivery_status}
                            </span>
                            
                            {order.shipping_label_url && (
                               <div className="flex flex-col items-end gap-1.5 w-full mt-2">
                                  {/* Label Button */}
                                  <button 
                                    onClick={() => window.open(order.shipping_label_url, '_blank')} 
                                    className="w-full bg-white border border-border text-foreground px-3 py-2 rounded-lg text-[10px] font-bold uppercase hover:bg-secondbg transition-colors flex items-center justify-center gap-2 shadow-sm"
                                  >
                                    <Printer size={12} /> Print Label
                                  </button>
                                  
                                  {/* Manifest Link - Mobile Safe, Clear CTA */}
                                  <div className="w-full flex items-center justify-center gap-1 bg-slate-50 border border-slate-100 px-3 py-2 rounded-lg">
                                    <FileText size={10} className="text-slate-400" />
                                    <a 
                                      href="https://app.shiprocket.in/manifests" 
                                      target="_blank" 
                                      className="text-[9px] font-black uppercase text-slate-500 hover:text-primary transition-colors underline decoration-dotted"
                                    >
                                      Generate Manifest
                                    </a>
                                  </div>
      ...
      {previewImage && (
        <div 
          ...
          onClick={() => setPreviewImage(null)} 
        >
          <div className="bg-card rounded-[1.5rem] p-2 max-w-sm w-full relative shadow-2xl animate-in zoom-in-95 border border-white/10 cursor-default" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setPreviewImage(null)} className="absolute top-4 right-4 z-50 p-2 bg-black/50 hover:bg-black/70 text-white rounded-full transition-all backdrop-blur-md">
              <X size={20} />
            </button>
            <div className="aspect-[3/5] w-full bg-black rounded-[1rem] overflow-hidden flex items-center justify-center relative">
